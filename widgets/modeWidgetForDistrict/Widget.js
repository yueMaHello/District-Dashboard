    ...
    function (declare, BaseWidget, Evented, _OnDijitClickMixin, _WidgetsInTemplateMixin,
        ...
        return declare([BaseWidget, _OnDijitClickMixin, _WidgetsInTemplateMixin, Evented], {
            ...
            startup: function() {
                ...
                d3.csv("./data/mode_by_district.csv",function(data){
                    modeMatrix = buildMatrixLookup(data);
                    return data;
                ...
                districtLayer.on('click',function(e){
                    ...
                    var autoOwnershipLine = modeMatrix[selectedZone];
                
                    google.charts.load('current', {'packages':['corechart']});
                    google.charts.setOnLoadCallback(drawChart);

                    function drawChart() {
                        console.log(autoOwnershipLine);

                        var array_keys = ["incomeRange","percentage"];
                        var incomeArray = new Array();
                        incomeArray.push(array_keys);
                        for (var key in autoOwnershipLine) {
                            incomeArray.push([key,parseFloat(autoOwnershipLine[key])]);
                        }
                        // example data format
                        // let exm = [
                        //     ['Task', 'Hours per Day'],
                        ...
                        var options = {'title':'Travel Mode', 'is3D': true, 'width':550, 'height':400,'backgroundColor':'#222222',titleTextStyle: {
                                color: 'white'
                        ...
                        document.getElementById('modeBarChart').style.margin= "-50px 0px 0px -80px";
                        var chart = new google.visualization.PieChart(document.getElementById('modeBarChart'));
